    ...
    for host in hosts:
        ...
        try:
            ...
            for port in ports:
                ...
                # Add empty plugin entries ( plugins output follows )
                port_data.extend(("","",""))
        ...
        # Add a row for each <elem> of <script> inside <hostscript>
        # Base keps the first elements empty (such as port etc.)
        base = [ip_address,host_name]+[""]*9
        try:
            hostscript = host.findall("hostscript")[0]
            for script in hostscript.findall("script"):
                plugin_name = script.attrib['id']
                
                plugin_output = script.attrib['output']
                host_data.append(base+[plugin_name,"output",plugin_output])

                for elem in script.findall("elem"):
                    elem_key = elem.attrib['key']
                    elem_val = elem.text
                    host_data.append(base+[plugin_name,elem_key,elem_val])
